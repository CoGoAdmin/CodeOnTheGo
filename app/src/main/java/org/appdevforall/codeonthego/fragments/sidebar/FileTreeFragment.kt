...
import org.appdevforall.codeonthego.databinding.LayoutEditorFileTreeBinding
...
import org.appdevforall.codeonthego.tasks.callables.FileTreeCallable.SortFileName
import org.appdevforall.codeonthego.tasks.callables.FileTreeCallable.SortFolder
import com.itsaky.androidide.utils.doOnApplyWindowInsets
import org.appdevforall.codeonthego.viewmodel.FileTreeViewModel
...
class FileTreeFragment : BottomSheetDialogFragment(), TreeNodeClickListener,
  ...
  private fun updateChevron(node: TreeNode) {
    if (node.viewHolder is org.appdevforall.codeonthego.adapters.viewholders.FileTreeViewHolder) {
      (node.viewHolder as org.appdevforall.codeonthego.adapters.viewholders.FileTreeViewHolder).updateChevron(node.isExpanded)
    }
  }
  ...
  private fun setLoading(node: TreeNode) {
    if (node.viewHolder is org.appdevforall.codeonthego.adapters.viewholders.FileTreeViewHolder) {
      (node.viewHolder as org.appdevforall.codeonthego.adapters.viewholders.FileTreeViewHolder).setLoading(true)
    }
  }
  ...
  private fun listFilesForNode(files: Array<File>, parent: TreeNode) {
    ...
    for (file in files) {
      ...
      node.viewHolder = org.appdevforall.codeonthego.adapters.viewholders.FileTreeViewHolder(context)
  ...
  fun listProjectFiles() {
    ...
    rootNode.viewHolder =
        org.appdevforall.codeonthego.adapters.viewholders.FileTreeViewHolder(requireContext())

    val projectRoot = TreeNode.root(projectDir)
    projectRoot.viewHolder = org.appdevforall.codeonthego.adapters.viewholders.FileTreeViewHolder(context)
    ...
    executeAsync(
        org.appdevforall.codeonthego.tasks.callables.FileTreeCallable(
            context,
            projectRoot,
            projectDir
        )
