...
import org.appdevforall.codeonthego.databinding.LayoutSearchProjectBinding
import com.itsaky.androidide.flashbar.Flashbar
import org.appdevforall.codeonthego.fragments.sheets.ProgressSheet
import org.appdevforall.codeonthego.handlers.EditorBuildEventListener
import org.appdevforall.codeonthego.handlers.LspHandler.connectClient
import org.appdevforall.codeonthego.handlers.LspHandler.destroyLanguageServers
...
import org.appdevforall.codeonthego.services.builder.GradleBuildService
import org.appdevforall.codeonthego.services.builder.GradleBuildServiceConnnection
import org.appdevforall.codeonthego.services.builder.gradleDistributionParams
...
import org.appdevforall.codeonthego.utils.RecursiveFileSearcher
...
abstract class ProjectHandlerActivity : BaseEditorActivity() {
  ...
  protected var mSearchingProgress: org.appdevforall.codeonthego.fragments.sheets.ProgressSheet? = null
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    editorViewModel._isSyncNeeded.observe(this) { isSyncNeeded:Boolean ->
      if (!isSyncNeeded) {
  ...
  override fun preDestroy() {
    ...
    if (org.appdevforall.codeonthego.lsp.IDELanguageClientImpl.isInitialized()) {
      org.appdevforall.codeonthego.lsp.IDELanguageClientImpl.shutdown()
    }
  ...
  protected open fun createFindInProjectDialog(moduleDirs: List<File>): AlertDialog? {
    ...
    builder.setPositiveButton(string.menu_find) { dialog, _ ->
      ...
      if (searchDirs.isEmpty()) {
        ...
        org.appdevforall.codeonthego.utils.RecursiveFileSearcher.searchRecursiveAsync(text, extensionList, searchDirs) { results ->
  ...
  private fun initLspClient() {
    if (!org.appdevforall.codeonthego.lsp.IDELanguageClientImpl.isInitialized()) {
      org.appdevforall.codeonthego.lsp.IDELanguageClientImpl.initialize(this as EditorHandlerActivity)
    }
    connectClient(org.appdevforall.codeonthego.lsp.IDELanguageClientImpl.getInstance())
  }

  open fun getProgressSheet(msg: Int): org.appdevforall.codeonthego.fragments.sheets.ProgressSheet? {
    doDismissSearchProgress()

    mSearchingProgress =
      org.appdevforall.codeonthego.fragments.sheets.ProgressSheet().also {
