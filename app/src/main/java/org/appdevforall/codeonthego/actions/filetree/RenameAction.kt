...
class RenameAction(context: Context, override val order: Int) :
  ...
  override suspend fun execAction(data: ActionData) {
    ...
    binding.name.editText!!.hint =
      context.getString(org.appdevforall.codeonthego.resources.R.string.new_name)
    binding.name.editText!!.setText(file.name)
    builder.setTitle(org.appdevforall.codeonthego.resources.R.string.rename_file)
    builder.setMessage(org.appdevforall.codeonthego.resources.R.string.msg_rename_file)
    builder.setView(binding.root)
    builder.setNegativeButton(android.R.string.cancel, null)
    builder.setPositiveButton(org.appdevforall.codeonthego.resources.R.string.rename_file) {
      dialogInterface,
      _ ->
      dialogInterface.dismiss()
      actionScope.launchAsyncWithProgress(
          configureFlashbar = { builder, cancelChecker ->
            builder.message(org.appdevforall.codeonthego.resources.R.string.please_wait)
          },
          action = { _, _ ->
            ...
            withContext(Dispatchers.Main) {
              flashMessage(
                  if (renamed) org.appdevforall.codeonthego.resources.R.string.renamed
                  else org.appdevforall.codeonthego.resources.R.string.rename_failed,
              ...
              if (lastHeld != null) {
                ...
                node.viewHolder =
                    org.appdevforall.codeonthego.adapters.viewholders.FileTreeViewHolder(context)
